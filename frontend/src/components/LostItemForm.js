...
const LostItemForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!form.checkValidity()) {
      ...
      try {
        const response = await fetch('http://127.0.0.1:5000/add_lost_item', {
          method: 'POST',
          headers: {
        ...
        window.location.href = '/redirected-page'; // Replace '/NavBar.js' with the desired URL
